...
<a class="btn btn-primary" href="{{route('ExameMedico.index')}}">Novo Exame</a>
...
<div class="col-lg-6">
  ...
  <tbody>
   ...
   <div class="material-switch pull-right">
      ...
      <td>{{$user->id }}</td>
       <td style="width: 73px;
    padding-right: 0px;"><img src="/imagens/misc/{{$user->avatar }}" style="width: 100%;
             max-width: 45px;
             height: auto;
             border-radius: 50%;"></td>
      <td>{{$nomeusuario= str_limit($user->nome_exame, 20) }}</td>
      <td>{{$sub= str_limit($user->sub_titulo,30) }}</td>
      ...
      <td>
          ...
          <a class="btn btn-danger" href="{{ route('DeleteExameAdmin.index', $user->id) }}"><i class="fa fa-trash-o"></i></a>
